...
public abstract class TPartStoredProcedure<H extends StoredProcedureParamHelper>
	...
	private static int readOnlyTxnRecordCounter = 0;
	private static long getlocktimeofreadonly  = 0;
	private static long getlocktimeoftotal = 0;

	// MODIFIED:
	public static void resetFinishedCount(){
		finishedCount.set(0);
	}
	...
	public void bookConservativeLocks() {
		...
		// MODIFIED:
		if(isReadOnly()){
			readOnlyTxnRecordCounter += getReadSet().size();
		}
	}
	...
	public SpResultSet execute() {
		try {
			...
			getlocktimeoftotal += Timer.getLocalTimer().getComponentTime("Get locks");
			if(isReadOnly())
				getlocktimeofreadonly += Timer.getLocalTimer().getComponentTime("Get locks");
			
			executeTransactionLogic();

			// MODIFIED: Remove commit for each Txns
			tx.commit();
			isCommitted = true;
			// System.out.println(tx.getTransactionNumber()+" ori commit");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void flushCaches(){
		cache.flushDitryCaches(plan,  cachedEntrySet);
		// tx.commit();
		// isCommitted = true;
		// System.out.println(tx.getTransactionNumber()+" commit");
	}
	...
	private void executeTransactionLogic() {
		// Set<PrimaryKey> readSet = plan.getReadSet();
		// Set<PrimaryKey> writeSet = plan.getWriteSet();
		...
		if (plan.isHereMaster()) {
			...
			if (pi != null) {
				...
				for (Entry<Integer, Set<PushInfo>> entry : pi.entrySet()) {
					...
					// System.out.println("push " + targetServerId + " " + pushingcount +" / " + counter);
				}
			}
...
//				}
		...
		if (counter % 10000 == 0){
			double AvgTotalLockTime = getlocktimeoftotal / counter;
			double AvgReadOnlyLockTime = getlocktimeofreadonly / counter;
			
			System.out.println("Tx:" + txNum + "'s Timer:\n" + timer.toString());
			System.out.println("Active Txn Num: " + Transaction.getActiveTxnNum() + " | Read-Only: " + Transaction.getActiveReadOnlyTxnNum());
			System.out.println("Get Lock Time: (total)" + AvgTotalLockTime + ", (Read-only)" + AvgReadOnlyLockTime + " Ratio: " + String.format("%.05f", AvgReadOnlyLockTime / AvgTotalLockTime));
			System.out.println("Read Lock Counter: " + lockCounter + " Of  Read-Only Txn: " + readOnlyTxnRecordCounter + " Ratio: " + String.format("%.05f",((double)lockCounter /  (double)readOnlyTxnRecordCounter)));
		}
		...
		//  cache.pushForward(plan, cachedEntrySet);
