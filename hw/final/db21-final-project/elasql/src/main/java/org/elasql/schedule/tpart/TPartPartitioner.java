...
import org.elasql.schedule.tpart.hermes.Greedy;
...
//				}
				...
				if ((batchingEnabled && batchedTasks.size() >= NUM_TASK_PER_SINK)
					...
					batchedTasks.clear();
					// System.out.println("clear batchedTasks");
				}
	...
	private void processBatch(List<TPartStoredProcedureTask> batchedTasks) {
		...
		// Transaction reordering with Hermes
		// Greedy g = new Greedy();
		// batchedTasks = new ArrayList<TPartStoredProcedureTask>(g.reorderWithHermes((HermesNodeInserter)inserter, graph, batchedTasks));
		// System.out.println("Greedy sorted batch size: " + batchedTasks.size());

		// Insert the batch of tasks
		inserter.insertBatch(graph, batchedTasks);
		// TPartStoredProcedure.resetFinishedCount();

		// long end = System.currentTimeMillis();      
		// System.out.println("Total Elapsed Time in milli seconds: " + (end - start));
...
//		collectGraphStatistics();
		...
		// } finally{
			// MODIFIED: Wating for batch completed
			// int a=0;
			// while(TPartStoredProcedure.getFinishedCount() < ((HermesNodeInserter) inserter).getLoadPerPart()[Elasql.serverId()]){
			// 	if(a!=TPartStoredProcedure.getFinishedCount()){
			// 		System.out.println("FinishedCount: "+
			// 			TPartStoredProcedure.getFinishedCount()+"/"+
			// 			((HermesNodeInserter) inserter).getLoadPerPart()[Elasql.serverId()]);
			// 		a = TPartStoredProcedure.getFinishedCount();
			// 	}
			// }
			// if(TPartStoredProcedure.getFinishedCount() >= ((HermesNodeInserter) inserter).getLoadPerPart()[Elasql.serverId()]){
			// 	System.out.println(TPartStoredProcedure.getFinishedCount()+" start batches flush");
...
//	}
	...
	private void dispatchToTaskMgr(Iterator<TPartStoredProcedureTask> plans) {
		...
		// List<Long> txnsId = new ArrayList<Long>();
		// List<Boolean> txnsIsReadOnly = new ArrayList<Boolean>();

		while (plans.hasNext()) {
			TPartStoredProcedureTask p = plans.next();
			// txnsId.add(p.getTxNum());
			// txnsIsReadOnly.add(p.isReadOnly());
		...
		// for(int i = 0; i < 5; i++){
		// 	if(txnsIsReadOnly.get(i)){
		// 		System.out.println(i + ". Txn: " +  txnsId.get(i) + " is Read-Only");
		// 	}else{
		// 		System.out.println(i + ". Txn: " +  txnsId.get(i) + " is NOT Read-Only");
		// 	}
		// }

		// System.out.println("-----Skip-----");

		// for(int i = txnsId.size() - 5; i < txnsId.size(); i++){
		// 	if(txnsIsReadOnly.get(i)){
		// 		System.out.println(i + ". Txn: " +  txnsId.get(i) + " is Read-Only");
		// 	}else{
		// 		System.out.println(i + ". Txn: " +  txnsId.get(i) + " is NOT Read-Only");
